...
const CountryDropDown = ({setCountry, country}) => {
    ...
    function CountriesList({ items }) {
        return items.map((item, index) => (
            <div 
            key={item.id} 
            style={{height: '34px', width: '88px', padding: '3px', marginBottom: !index ? '0.25rem' : ''}}
            className={`${
                (selectedTab === item.id) 
            ...
            >
                ...
                <label  style={{marginRight: '1.4375rem', fontSize: '16px', marginTop: '0.125rem'}} className={`${(selectedTab === item.id) ? "text-white" : "text-gray-subNavbar"} font-spoqaBold cursor-pointer`}>{item.text}</label>            </div>
        ));
    }
    return (
        <div style={{width: '94px', borderRadius: '20px'}} className="p-1 relative flex flex-col items-center bg-white shadow-plain">
            <div style={{width: '20px', height: '10px', marginTop: '-10px', marginLeft: '10px'}} className="absolute top-0 -mt-2 w-4 overflow-hidden inline-block">
