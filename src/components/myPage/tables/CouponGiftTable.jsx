...
const CouponGiftTable = ({array, titleArray, checkedState, setCheckedState, isPopupOpen, setPopupOpen, cardHeight = '33.75rem', isButtonGradient = true, hasLeftInput = true, hasExtraColumns = false, hasButton = true, containerBackground}) => {

    function Cells({ items, titles }) {
        const detailButton =  (
            <button style={{width: '20.25rem', height: '14.0625rem', padding: '1px'}} className="flex items-center justify-center rounded-lg bg-orange-d43712 hover:opacity-75">
            ...
            <div style={{height: cardHeight, width: `${!hasButton ? '73.875rem' : ''}` , paddingRight: '1.875rem', margin: '1.875rem', marginTop: `${index ? '0': '1.0625rem'}`, background: `${containerBackground && index % 2 ? containerBackground : ''}`}} className="p-12 bg-gray-fefefe font-spoqa text-14px tracking-tight text-gray-r585858 h-56px border-b border-gray-dddddd flex items-center justify-between shadow-subNavbar rounded-2xl">
                <div style={{marginTop: '-0.5rem'}} className="flex flex-wrap items-center w-full h-full">
                    ...
                    <div style={{width: ''}}>
                        ...
                        {titles?.[3] && (
                        <div style={{WebkitTextStroke:"0.2px", fontSize: '2.625rem', color: '#7b7b7b'}} className="w-full font-spoqa text-left text-gray-r7b7b7b">
