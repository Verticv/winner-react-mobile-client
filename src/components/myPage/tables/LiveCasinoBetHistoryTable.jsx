            ...
            <div style={{height: cardHeight, width: `${!hasButton ? '73.875rem' : ''}` , paddingRight: '1.875rem', margin: '1.875rem', marginTop: `${index ? '0': '1.875rem'}`, background: `${containerBackground && index % 2 ? containerBackground : ''}`}} className="p-12 bg-gray-fefefe font-spoqa text-14px tracking-tight text-gray-r585858 h-56px border-b border-gray-dddddd flex items-center justify-between shadow-subNavbar rounded-2xl">
                <div style={{marginTop: '-0.5rem'}} className="flex flex-wrap items-center w-full h-full">
                    ...
                    <div style={{width: '52.3125rem'}}>
                        {titles?.[0] && (<div style={{WebkitTextStroke:"0.2px", marginBottom: '0.3125rem', fontSize: '2.625rem', color: titles?.[0]?.isRed ? "#d52e2e" : ""}} className="w-full font-spoqa text-left text-gray-r7b7b7b"><span className='font-spoqaMedium text-gray-r585858' style={{fontSize: '2.625rem'}}>{titles[0].text} : </span> {item.number}</div>)} 
                        {titles?.[1] && (<div style={{WebkitTextStroke:"0.2px", fontSize: '2.625rem', marginBottom: '0.3125rem', color: titles?.[1]?.isRed ? "#d52e2e" : ""}} className="w-full font-spoqa text-left text-gray-r7b7b7b"><span className='font-spoqaMedium text-gray-r585858' style={{fontSize: '2.625rem'}}>{titles[1].text} : </span> {item.time}</div>)} 
                        {titles?.[2] && (
                        <div style={{WebkitTextStroke:"0.2px", marginBottom: '0.3125rem', fontSize: '2.625rem', color: titles?.[3]?.isRed ? item.type?.includes("+") ? "#d52e2e" : item.type.includes("-") ? "#0056a6" : "#7b7b7b" : "#7b7b7b"}} className="w-full font-spoqa text-left text-gray-r7b7b7b">
                        ...
                        {titles?.[3] && (
                        <div style={{WebkitTextStroke:"0.2px", fontSize: '2.625rem', marginBottom: '0.3125rem', color: titles?.[3]?.isRed ? item.name.includes("+") ? "#d52e2e" : item.name.includes("-") ? "#0056a6" : "#7b7b7b" : "#7b7b7b"}} className="w-full font-spoqa text-left text-gray-r7b7b7b">
                        ...
                        {titles?.[4] && (
                        <div style={{WebkitTextStroke:"0.2px", fontSize: '2.625rem', marginBottom: '0.3125rem', color: titles?.[3]?.isRed ? item.amount.includes("+") ? "#d52e2e" : item.amount.includes("-") ? "#0056a6" : "#7b7b7b" : "#7b7b7b"}} className="w-full font-spoqa text-left text-gray-r7b7b7b">
                        ...
                        {titles?.[5] && (<div style={{WebkitTextStroke:"0.2px", fontSize: '2.625rem', marginBottom: '0.3125rem', color: item.profit.includes("+") ? "#d52e2e" : "#585858"}} className="w-full font-spoqa text-left text-gray-r7b7b7b"><span className='font-spoqaMedium text-gray-r585858' style={{fontSize: '2.625rem'}}>{titles[5].text} : </span> {item.profit}</div>)} 
                        {titles?.[6] && (<div style={{WebkitTextStroke:"0.2px", fontSize: '2.625rem',  marginBottom: '0.3125rem', color: item.status === "승" ? "#d52e2e" : "#585858"}} className="w-full font-spoqa text-left text-gray-r7b7b7b"><span className='font-spoqaMedium text-gray-r585858' style={{fontSize: '2.625rem'}}>{titles[6].text} : </span> {item.status}</div>)} 
                    </div>
                </div>
                {isButtonGradient ? 
                 ...
                 : hasButton &&
                    <button style={{width: '10.875rem', height: '10.875rem',  padding: '1px', backgroundColor: item?.buttonColor || '#0056a6'}} className="flex items-center justify-center rounded-xl hover:opacity-75">
                        <div className="flex w-full h-full items-center justify-center flex-wrap rounded-xl cursor-pointer">
                            <span style={{fontSize: '2.8125rem', lineHeight: '1.1', width: '10.875rem'}} className="w-full -mt-2 font-font-spoqaMedium tracking-tight text-white"> {item?.buttonColor === '#0056a6' ? <div> {item.buttonText ? item.buttonText : <><p>정산</p> <p>완료</p></>} </div> : item.buttonText ||'진행중'} </span>
